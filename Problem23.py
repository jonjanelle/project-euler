...
def getType(n):
    x = sum(getDivSet(n))
    if x > n:       #Abundant number
        return 'a'
    elif x == n:    #Perfect number
        return 'p'
    else:           #Deficient number
        return 'd'
...
def noNonAbSum():
    abList = listAbundant(28123).sort()
    abCnt = len(abList)
    naSum = 0
    for i in range(0, abCnt-1):
        for j in range(i+1,len(abCnt)):
            pass
